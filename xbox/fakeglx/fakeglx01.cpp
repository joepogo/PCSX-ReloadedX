...
//extern void SetGUID3DDevice(LPDIRECT3DDEVICE8 pD3DDevice);
...
//				delete tex
	...
	subImage_s* GetSubImageCache(IDirect3DDevice8* pDevice, IDirect3DTexture8* pMipMap, int level, int iNum)
	{
		...
		pMipMap->GetLevelDesc(level, &pNewSubImage->desc); 
	
		// Check the formats and change bytesPerPixel accordingly
		switch ( pNewSubImage->desc.Format ) 
		{
			case D3DFMT_P8:
				d3dFormat = D3DFMT_P8;
				pNewSubImage->iBytes = 1;
			break;

			case D3DFMT_AL8:
				d3dFormat = D3DFMT_AL8;
				pNewSubImage->iBytes = 1;
			break;
		...
		pDevice->CreateImageSurface(pNewSubImage->desc.Width, pNewSubImage->desc.Height,
									d3dFormat, &pNewSubImage->pTexture);
...
#endif
		if(m_bScissorTest)
			m_pD3DDev->SetScissors(1, FALSE, &m_ScissorRect);
...
//				m_pD3DDev->SetRenderState(D3DRS_FOGVERTEXMODE, D3DFOG_EXP2);
	...
	inline void glScissor (GLint x, GLint y, GLsizei width, GLsizei height)
	{
		...
		y = m_glViewPortHeight - (height + y);
	
		m_ScissorRect.x1 = x;
		m_ScissorRect.y1 = y;

		m_ScissorRect.x2 = x + width;
		m_ScissorRect.y2 = y + height;
	}
...
// to texture swizzleing / unswizzleing.
		...
		GLenum /* format */, GLenum /* type */, const char* compatablePixels, int compatablePixelsPitch)
	{
		...
		cachedSubImage = m_textures.GetSubImageCache(m_pD3DDev, pMipMap, level, m_textures.GetCurrentID());
		...
		for (y = yoffset; y < (yoffset + height); y++)
		{
			...
			{
				iElement = lockedRectSubImg.Pitch * y + cachedSubImage->iBytes * x; 
					
				dstdata[iElement] = srcdata[0];
				dstdata[iElement + 1] = srcdata[1];
				dstdata[iElement + 2] = srcdata[2];
				dstdata[iElement + 3] = srcdata[3];

				srcdata += cachedSubImage->iBytes;
			}
		}
		...
		&rect,						 // pRect,	
		lr.pBits,					 // pDest,
		cachedSubImage->desc.Width,	 // Width,
		cachedSubImage->desc.Height, // Height,
		&point,						 // pPoint,
		cachedSubImage->iBytes);	 // BytesPerPixel
	...
	void SetGLRenderState()
	{
		...
		if (!m_PolyOffsetSwitched)
		{
			...
			else
			{
				...
				// (see comment in 
				m_pD3DDev->SetRenderState(D3DRS_ZBIAS, /*8*/0);
			}
...
void glGenTextures (GLsizei n, GLuint *textures)
{
	gFakeGL->glGenTextures(n, textures);
}

void glTexCoord2fv (const GLfloat *v)
{
	gFakeGL->glTexCoord2f(v[0], v[1]);
}

#endif //_USEFAKEGLX_01