...
#else //_XBOX
// Moved into XBoxMain.c as we need these for the GUI
#if 0
 wglCreateContext(/*dcGlobal*/NULL);
 wglMakeCurrent(/*dcGlobal, objectRC*/NULL, NULL);
#endif
#endif
#if defined (_MACGL)
 BringContextForward();
...
#if 0 // FIXME: Screws up the scissors when returning
	  //        from XBox GUI when enabled
...
#else 
  // The above does nothing in our wrapper.
  // We need turn on/off golbal full screen AA 
  // in our present parameters in FakeGLx

  // TODO: Control from config
  // Currently AA is on by default
...
#ifndef _XBOX // We don't need these, our default D3D values
			  // are initialized in our FGLx wrapper
