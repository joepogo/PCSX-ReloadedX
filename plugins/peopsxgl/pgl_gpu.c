...
#if defined (_MACGL)
#include "drawgl.h"
#elif !defined(_MACGL) && !defined(_WINDOWS) && !defined(_XBOX)
...
#ifndef _XBOX // Not needed on XBox, if we need/want blur will use a D3D shader
int             iBlurBuffer=0;
#endif
int             iScanBlend=0;
int             iRenderFVR=0;
...
#elif _XBOX
 sprintf(szTxt,"D:\\snap\\pcsxr%04d.txt",iNum);
#else
...
#elif _XBOX
   sprintf(filename,"D:\\snap\\pcsxr%04d.bmp",snapshotnr);
#else
...
long CALLBACK GPUopen(HWND hwndGPU)                    
{
 ...
 if(bChangeWinMode)                                    // user wants to change fullscreen/window mode?
  ...
  {
   PGL_ReadConfig();                                   // -> read config from registry
...
long CALLBACK Plugin_GPUopen(/*HWND hwndGPU*/void)
{
 ...
 PGL_ReadConfig();                                     // read text file for config
...
long CALLBACK Plugin_GPUclose(int iDestoryContext)     // XBOX CLOSE
{
 GLcleanup(iDestoryContext);                           // close OGL
...
#ifndef _XBOX // Not needed on XBox, if we need/want scanlines we will use a D3D shader
...
#ifndef _XBOX // Not needed on XBox, if we need/want blur eefect we will use a D3D shader
...
#endif
#else
 wglMakeCurrent(NULL, NULL);						   // -> make context current again
#endif //_XBOX
...
#ifndef _XBOX // Not needed on XBox, if we need/want blur and scanlines we will use a D3D shader
...
#ifndef _XBOX // Not needed on XBox, if we need/want scanlines and blur we will use a D3D shader
...
void CheckVRamReadEx(int x, int y, int dx, int dy)
{
 ...
 if(!pGfxCardScreen)
  {
#ifndef _XBOX // Our wrapper handles this
...
{
 Plugin_GPUwriteDataMem(&gdata,1);
}
 ...
 do
  {
   ...
   if(count>0) Plugin_GPUwriteDataMem(&baseAddrL[dmaMem>>2],count);
...
long CALLBACK Plugin_GPUfreeze(uint32_t ulGetFreezeData,GPUFreeze_t * pF)
{
 ...
 Plugin_GPUwriteStatus(ulStatusControl[0]);
 Plugin_GPUwriteStatus(ulStatusControl[1]);
 Plugin_GPUwriteStatus(ulStatusControl[2]);
 Plugin_GPUwriteStatus(ulStatusControl[3]);
 Plugin_GPUwriteStatus(ulStatusControl[8]);
 Plugin_GPUwriteStatus(ulStatusControl[6]);
 Plugin_GPUwriteStatus(ulStatusControl[7]);
 Plugin_GPUwriteStatus(ulStatusControl[5]);
 Plugin_GPUwriteStatus(ulStatusControl[4]);
...
////////////////////////////////////////////////////////////////////////
#ifndef _XBOX // Do we care about getting screen captures on XBox??
			  // If we do, an XDK back buffer capture fuction is prob better than this
...
void /*CALLBACK*/ Plugin_GPUshowScreenPic(unsigned char * pMem)
{
 DestroyPic();
 if(pMem==0) return;
 CreatePic(pMem);
