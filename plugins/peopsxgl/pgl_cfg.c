...
#else
 char pgl_cfgfile[MAXPATHLEN];

 strncpy(pgl_cfgfile, PcsxrDir, 200);
 strcat(pgl_cfgfile, "configs\\gpuPeopsMesaGL.cfg");

 in = fopen(pgl_cfgfile, "rb"); 
#endif
...
void PGL_ReadConfig(void)                                  // read config (linux file)
{
 iResX=640;
 iResY=480;
 iColDepth=16;
