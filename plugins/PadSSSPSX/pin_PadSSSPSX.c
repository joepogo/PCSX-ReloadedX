...
static void (CALLBACK *gpuVisualVibration)(uint32_t, uint32_t) = NULL;

#ifdef _XBOX
#define	PSX_PAD_L2       0x0001
#define	PSX_PAD_R2       0x0002
#define	PSX_PAD_L1       0x0004
#define	PSX_PAD_R1       0x0008

#define	PSX_PAD_TRIANGLE 0x0010
#define	PSX_PAD_CIRCLE   0x0020
#define	PSX_PAD_CROSS    0x0040
#define	PSX_PAD_SQUARE   0x0080

#define	PSX_PAD_SELECT   0x0100
#define	PSX_PAD_L3       0x0200
#define	PSX_PAD_R3       0x0400
#define	PSX_PAD_START    0x0800

#define	PSX_PAD_UP       0x1000
#define	PSX_PAD_RIGHT    0x2000
#define	PSX_PAD_DOWN     0x4000
#define	PSX_PAD_LEFT     0x8000
#endif
...
#else
	global.config.buttons[0][K_XBOX_A] = PSX_PAD_CROSS;
	global.config.buttons[0][K_XBOX_X] = PSX_PAD_SQUARE;
	global.config.buttons[0][K_XBOX_Y] = PSX_PAD_TRIANGLE;
	global.config.buttons[0][K_XBOX_B] = PSX_PAD_CIRCLE;
	global.config.buttons[0][K_XBOX_DPAD_UP] = PSX_PAD_UP;
	global.config.buttons[0][K_XBOX_DPAD_DOWN] = PSX_PAD_DOWN;
	global.config.buttons[0][K_XBOX_DPAD_LEFT] = PSX_PAD_LEFT;
	global.config.buttons[0][K_XBOX_DPAD_RIGHT] = PSX_PAD_RIGHT;
	global.config.buttons[0][K_XBOX_BACK] = PSX_PAD_SELECT;
	global.config.buttons[0][K_XBOX_START] = PSX_PAD_START;
	global.config.buttons[0][K_XBOX_LTRIG] = PSX_PAD_L1;
	global.config.buttons[0][K_XBOX_RTRIG] = PSX_PAD_R1;
	global.config.buttons[0][K_XBOX_WHITE] = PSX_PAD_L2;
	global.config.buttons[0][K_XBOX_BLACK] = PSX_PAD_R2;
#endif
...
#ifdef _XBOX
#define SetButtonValue(name, var) \
	fprintf(f, "%s = %s\n", name, var);
#endif

static void SaveConfig (void) // TODO : Support Pad 2 as well
{
#ifndef _XBOX
	HKEY myKey;
	DWORD myDisp;
	...
	strncpy(cfgPath, PcsxrDir, MAXPATHLEN);
	strcat(cfgPath, "configs\\controls.cfg");

	f = fopen(cfgPath, "w");
	if (f == NULL) return;

	for (i = 0; i < sizeof(Buttons)/sizeof(sButton); i++)
	{
		if(Buttons[i].dwButton == global.config.buttons[0][K_XBOX_A])
			SetButtonValue("Gamepad_XBox_A", Buttons[i].strButtonName);
	}

	for (i = 0; i < sizeof(Buttons)/sizeof(sButton); i++)
	{
		if(Buttons[i].dwButton == global.config.buttons[0][K_XBOX_X])
			SetButtonValue("Gamepad_XBox_X", Buttons[i].strButtonName);
	}

	for (i = 0; i < sizeof(Buttons)/sizeof(sButton); i++)
	{
		if(Buttons[i].dwButton == global.config.buttons[0][K_XBOX_Y])
			SetButtonValue("Gamepad_XBox_X", Buttons[i].strButtonName);
	}

	for (i = 0; i < sizeof(Buttons)/sizeof(sButton); i++)
	{
		if(Buttons[i].dwButton == global.config.buttons[0][K_XBOX_B])
			SetButtonValue("Gamepad_XBox_B", Buttons[i].strButtonName);
	}

	for (i = 0; i < sizeof(Buttons)/sizeof(sButton); i++)
	{
		if(Buttons[i].dwButton == global.config.buttons[0][K_XBOX_START])
			SetButtonValue("Gamepad_XBox_Start", Buttons[i].strButtonName);
	}

	for (i = 0; i < sizeof(Buttons)/sizeof(sButton); i++)
	{
		if(Buttons[i].dwButton == global.config.buttons[0][K_XBOX_BACK])
			SetButtonValue("Gamepad_XBox_Back", Buttons[i].strButtonName);
	}

	for (i = 0; i < sizeof(Buttons)/sizeof(sButton); i++)
	{
		if(Buttons[i].dwButton == global.config.buttons[0][K_XBOX_DPAD_UP])
			SetButtonValue("Gamepad_XBox_Dpad_Up", Buttons[i].strButtonName);
	}

	for (i = 0; i < sizeof(Buttons)/sizeof(sButton); i++)
	{
		if(Buttons[i].dwButton == global.config.buttons[0][K_XBOX_DPAD_DOWN])
			SetButtonValue("Gamepad_XBox_Dpad_Down", Buttons[i].strButtonName);
	}

	for (i = 0; i < sizeof(Buttons)/sizeof(sButton); i++)
	{
		if(Buttons[i].dwButton == global.config.buttons[0][K_XBOX_DPAD_LEFT])
			SetButtonValue("Gamepad_XBox_Dpad_Left", Buttons[i].strButtonName);
	}

	for (i = 0; i < sizeof(Buttons)/sizeof(sButton); i++)
	{
		if(Buttons[i].dwButton == global.config.buttons[0][K_XBOX_DPAD_RIGHT])
			SetButtonValue("Gamepad_XBox_Dpad_Right", Buttons[i].strButtonName);
	}

	for (i = 0; i < sizeof(Buttons)/sizeof(sButton); i++)
	{
		if(Buttons[i].dwButton == global.config.buttons[0][K_XBOX_LTRIG])
			SetButtonValue("Gamepad_XBox_LTrig", Buttons[i].strButtonName);
	}

	for (i = 0; i < sizeof(Buttons)/sizeof(sButton); i++)
	{
		if(Buttons[i].dwButton == global.config.buttons[0][K_XBOX_RTRIG])
			SetButtonValue("Gamepad_XBox_RTrig", Buttons[i].strButtonName);
	}

	for (i = 0; i < sizeof(Buttons)/sizeof(sButton); i++)
	{
		if(Buttons[i].dwButton == global.config.buttons[0][K_XBOX_WHITE])
			SetButtonValue("Gamepad_XBox_White", Buttons[i].strButtonName);
	}

	for (i = 0; i < sizeof(Buttons)/sizeof(sButton); i++)
	{
		if(Buttons[i].dwButton == global.config.buttons[0][K_XBOX_BLACK])
			SetButtonValue("Gamepad_XBox_Black", Buttons[i].strButtonName);
	}
...
#ifdef _XBOX
static void GetButtonValue(char *src, char *name, char *outvar)
{
	char *tmp;
...
static void LoadConfig (void) // TODO : Support Pad 2 as well
{
#ifndef _XBOX
	HKEY myKey;
	DWORD type, size;
...
#else
	if (stat(cfgPath, &buf) == -1)
	{
		// Uh oh, no cfg file so write ours out using default values
		SaveConfig();
		return;
	}
	...
	GetButtonValue(data, "Gamepad_XBox_A", strButton);

	for (i = 0; i < sizeof(Buttons)/sizeof(sButton); i++)
	{
		if(!strcmp(Buttons[i].strButtonName, strButton))
			global.config.buttons[0][K_XBOX_A] = Buttons[i].dwButton;		
	}
	
	GetButtonValue(data, "Gamepad_XBox_X", strButton);

	for (i = 0; i < sizeof(Buttons)/sizeof(sButton); i++)
	{
		if(!strcmp(Buttons[i].strButtonName, strButton))
			global.config.buttons[0][K_XBOX_X] = Buttons[i].dwButton;		
	}

	GetButtonValue(data, "Gamepad_XBox_Y", strButton);

	for (i = 0; i < sizeof(Buttons)/sizeof(sButton); i++)
	{
		if(!strcmp(Buttons[i].strButtonName, strButton))
			global.config.buttons[0][K_XBOX_Y] = Buttons[i].dwButton;		
	}

	GetButtonValue(data, "Gamepad_XBox_B", strButton);

	for (i = 0; i < sizeof(Buttons)/sizeof(sButton); i++)
	{
		if(!strcmp(Buttons[i].strButtonName, strButton))
			global.config.buttons[0][K_XBOX_B] = Buttons[i].dwButton;		
	}

	GetButtonValue(data, "Gamepad_XBox_Start", strButton);

	for (i = 0; i < sizeof(Buttons)/sizeof(sButton); i++)
	{
		if(!strcmp(Buttons[i].strButtonName, strButton))
			global.config.buttons[0][K_XBOX_START] = Buttons[i].dwButton;		
	}

	GetButtonValue(data, "Gamepad_XBox_Back", strButton);

	for (i = 0; i < sizeof(Buttons)/sizeof(sButton); i++)
	{
		if(!strcmp(Buttons[i].strButtonName, strButton))
			global.config.buttons[0][K_XBOX_BACK] = Buttons[i].dwButton;		
	}

	GetButtonValue(data, "Gamepad_XBox_Dpad_Up", strButton);

	for (i = 0; i < sizeof(Buttons)/sizeof(sButton); i++)
	{
		if(!strcmp(Buttons[i].strButtonName, strButton))
			global.config.buttons[0][K_XBOX_DPAD_UP] = Buttons[i].dwButton;		
	}

	GetButtonValue(data, "Gamepad_XBox_Dpad_Down", strButton);

	for (i = 0; i < sizeof(Buttons)/sizeof(sButton); i++)
	{
		if(!strcmp(Buttons[i].strButtonName, strButton))
			global.config.buttons[0][K_XBOX_DPAD_DOWN] = Buttons[i].dwButton;		
	}

	GetButtonValue(data, "Gamepad_XBox_Dpad_Left", strButton);

	for (i = 0; i < sizeof(Buttons)/sizeof(sButton); i++)
	{
		if(!strcmp(Buttons[i].strButtonName, strButton))
			global.config.buttons[0][K_XBOX_DPAD_LEFT] = Buttons[i].dwButton;		
	}

	GetButtonValue(data, "Gamepad_XBox_Dpad_Right", strButton);

	for (i = 0; i < sizeof(Buttons)/sizeof(sButton); i++)
	{
		if(!strcmp(Buttons[i].strButtonName, strButton))
			global.config.buttons[0][K_XBOX_DPAD_RIGHT] = Buttons[i].dwButton;		
	}

	GetButtonValue(data, "Gamepad_XBox_LTrig", strButton);

	for (i = 0; i < sizeof(Buttons)/sizeof(sButton); i++)
	{
		if(!strcmp(Buttons[i].strButtonName, strButton))
			global.config.buttons[0][K_XBOX_LTRIG] = Buttons[i].dwButton;		
	}

	GetButtonValue(data, "Gamepad_XBox_RTrig", strButton);

	for (i = 0; i < sizeof(Buttons)/sizeof(sButton); i++)
	{
		if(!strcmp(Buttons[i].strButtonName, strButton))
			global.config.buttons[0][K_XBOX_RTRIG] = Buttons[i].dwButton;		
	}

	GetButtonValue(data, "Gamepad_XBox_White", strButton);

	for (i = 0; i < sizeof(Buttons)/sizeof(sButton); i++)
	{
		if(!strcmp(Buttons[i].strButtonName, strButton))
			global.config.buttons[0][K_XBOX_WHITE] = Buttons[i].dwButton;		
	}

	GetButtonValue(data, "Gamepad_XBox_Black", strButton);

	for (i = 0; i < sizeof(Buttons)/sizeof(sButton); i++)
	{
		if(!strcmp(Buttons[i].strButtonName, strButton))
			global.config.buttons[0][K_XBOX_BLACK] = Buttons[i].dwButton;		
	}
...
#ifdef _XBOX
void setpressure( int pad, unsigned int value )
{
	int i ;
	for ( i = 0 ; i < 16 ; i++ )
	{
		if ( (1<<i) & value )
		{
			global.padPress[pad][i] = 0 ;
		}
		else
		{
			if (global.padPress[pad][i] == 0)
				global.padPress[pad][i] = GetTickCount();
		}
	}
}
#endif
...
static void UpdateState (const int pad)
{
	...
	padstatus = ReadXBoxButtons( 0 );
	checkAnalog( 0, padstatus&0x40000 );
	global.padStat[0] =  padstatus & 0xFFFF;
	setpressure( 0, global.padStat[0] );

	padstatus = ReadXBoxButtons( 1 );
	checkAnalog( 1, padstatus&0x40000 );
	global.padStat[1] =  padstatus & 0xFFFF;
	setpressure( 1, global.padStat[1] );

	ReadXBoxThumbSticks(0, &(lastport1.leftJoyX), &(lastport1.leftJoyY), &(lastport1.rightJoyX), &(lastport1.rightJoyY));
	ReadXBoxThumbSticks(1, &(lastport2.leftJoyX), &(lastport2.leftJoyY), &(lastport2.rightJoyX), &(lastport2.rightJoyY));
	...
	{
		global.padVibF[pad][3] = vib1;
		SetXBoxRumble( pad, (global.padVibF[pad][1]+0x20) << 8 );
	}
#endif
}
...
s32 CALLBACK Plugin_PADinit (u32 flags)
{
	return 0;
}
...
static int n_open = 0;
s32 CALLBACK Plugin_PADopen (/*HWND hWnd*/)
{
#ifndef _XBOX
...
void CALLBACK Plugin_PADclose (void)
{
	if (--n_open == 0)
#ifndef _XBOX
		ReleaseDirectInput();
...
u8 CALLBACK Plugin_PADstartPoll (int pad)
{
	global.curPad = pad -1;
	global.curByte = 0;
	return 0xff;
...
u8 CALLBACK Plugin_PADpoll (u8 value)
{
...
long CALLBACK Plugin_PADreadPort1 (PadDataS* pads)
{
	...
	if ((global.padID[0] & 0xf0) == 0x40)
	{
		pads->rightJoyX = pads->rightJoyY = pads->leftJoyX = pads->leftJoyY = 127 ;
		pads->controllerType = 4;
	}
	else
	{
		pads->controllerType = 7;
		ReadXBoxThumbSticks(0, &(pads->leftJoyX), &(pads->leftJoyY), &(pads->rightJoyX), &(pads->rightJoyY));
	}

	wasKeypressed1 = ((pads->buttonStatus&0xFFFF) > 0 ) ? 1 : 0 ;
...
long CALLBACK Plugin_PADreadPort2 (PadDataS* pads)
{
	...
	{
		pads->controllerType = 7;
		ReadXBoxThumbSticks(1, &(pads->leftJoyX), &(pads->leftJoyY), &(pads->rightJoyX), &(pads->rightJoyY));
	}

	wasKeypressed2 = ((pads->buttonStatus&0xFFFF) > 0 ) ? 1 : 0 ;
